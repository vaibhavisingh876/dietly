              ...
              {navLinks.map((link) => {
                ...
                return (
                  <button
                    ...
                    className={`relative px-5 py-2.5 rounded-full font-bold transition-all duration-300 text-sm ${
                      isActive
                        ? "text-white"
              ...
              {isLoggedIn ? (
                <>
                  <button
                    ...
                    className={`relative px-4 py-2.5 rounded-full font-semibold transition-all duration-300 text-white flex items-center gap-2 ${
                      activeTab === "Profile"
                  ...
                  <button
                    ...
                    className="relative p-2.5 rounded-full transition-all duration-300 text-gray-600 hover:bg-red-50 hover:text-red-700"
                    title="Logout"
                  >
        ...
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <div className="absolute top-24 left-1/2 transform -translate-x-1/2 w-11/12 max-w-md bg-white rounded-3xl shadow-2xl p-6 animate-slideDown">
            <div className="space-y-3">
              {[...navLinks, ...authLinks.filter((link) => (isLoggedIn ? link.isAuth : !link.isAuth))].map(
                (link) => {
                  const IconComponent = link.icon;
                  const isActive = activeTab === link.name;
              ...
              {isLoggedIn && (
                <button
                  onClick={handleLogout}
                  className="w-full flex items-center gap-3 px-6 py-4 rounded-2xl font-bold text-red-600 hover:bg-red-50"
                >
                  <LogOut className="w-5 h-5" />
                  Logout
                </button>
